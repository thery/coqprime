...
reasonable when dealing with small numbers, dealing with larger number was problematic.
...
for $p > 2$, $2^p-1$ is prime if and only if $S_{p-2} = 0\, \mod\, M_p $.
\end{theorem}
 ...
 In Section 2, we present the default arithmetic of {\sc Coq}.
 Our representation of numbers is described in Section 3. In Section 4, we 
 ...
 In Section 6, we detail some further experiments that we have carried out based on
 our library.