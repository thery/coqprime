...
* A C program ```pocklington```  that generates Pocklington certificates (this program is based on [ECM](hhttps://gitlab.inria.fr/zimmerma/ecm)). An ocaml program ```o2v``` that turns a certificate generated by [primo](https://www.ellipsa.eu) into Coq files. These programs are in
[gencertif](./gencertif).
