...
Figure~\ref{fig:TimeComp} illustrates this on some small examples 
($P_{150}$ is a prime number of 150 digits when written
...
\label{fig:TimeComp}
\end{figure} 
Applying the two-level approach to larger numbers 
(> 1000 digits) made us realize
...
The fact that our arithmetic is a modular one is not a real limitation,
since most computations with large numbers can be rephrased in term of 
modular arithmetic. 
This is true in particular for the examples we are
interested in. 
More precisely, Condition 4 of Theorem \ref{lehmer} can be expressed 
with modulo as
$$
...
\begin{theorem}
Let $M_p$ be $2^p-1$ and let  ($S_n$) be recursively defined by 
$S_0= 4\, \mod\, M_p$ and $S_{n+1} = S_n^2 - 2\, \mod\, M_p$,
