...
Lemma to_Z_cons a l : ([a::l] = phi a + wB * [l])%Z.
Proof. auto. Qed.
...
Ltac nrom := 
 ...
 try change (phi max_int) with (wB - 1)%Z;
...
Proof.
assert (Hi0: (forall i n, Int31Op.to_Z i = Zpower_nat 2 n ->  
              match 2 * i ?= 0 with
              | Eq => n = 30%nat
              | Lt => False
...
assert (H: forall p1 i j n, phi i = Zpower_nat 2 n -> (phi j < phi i ->
   to_Z (positive_aux_to_num p1 i j) = Zpos p1 * phi i + phi j)%Z).
...
generalize (to_Z_pos n2); case Zcompare_spec; auto with zarith;
...
generalize (to_Z_pos (a::n1)); case Zcompare_spec; auto with zarith;
...
replace (phi (succ xh)) with (phi xh + 1)%Z; try ring.
case (to_Z_bounded a); nrom; intros H1a H2a.
...
rewrite Zplus_mod, Zmult_mod, Zmod_mod, <- Zmult_mod, <- Zmult_assoc,
        Zmult_mod.
unfold m.
generalize Hmm'; rewrite mul_spec; nrom; intros HH1; rewrite (Zmult_comm (phi m')), HH1.
...
Lemma reduce_aux_correct n t :
  [reduce_aux_num n t] * (Zpower_nat wB n) mod [M] = [t]  mod [M].
Proof.
...
Lemma reduce_correct n t :
  [reduce_num n t] * (Zpower_nat wB n) mod [M] = [t]  mod [M].
Proof.
assert (H:= M_pos).
...
Lemma r_square_lt n : 0 <= [r_square n] < [M].
Proof.
unfold r_square.
...
Proof.
case (Zmod_le_first (Zpower_nat wB n * Zpower_nat wB n) [M]); auto with zarith.
Qed.
...
assert (F1 : 0 <= [Z_to_num (p mod [M])] < [M]).
  rewrite num_to_Z; auto with zarith.
  rewrite Zmod_small; auto with zarith.
  case (Zmod_le_first p [M]); auto with zarith.
...
case (Zmod_le_first (Zpower_nat wB n * Zpower_nat wB n) [M]); 
  auto with zarith.
case (Zmod_le_first (p mod [M]) [M]); auto with zarith.
case (Zmod_le_first p [M]); auto with zarith.
apply reduce_bound_small.
