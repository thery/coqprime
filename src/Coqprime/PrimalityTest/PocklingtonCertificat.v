...
Definition mkProd_pred (l:dec_prime) :=
  fold_right (fun (k:positive*positive) r =>
    if ((snd k) ?= 1)%P then r else times (pow (fst k) (Pos.pred (snd k))) r)
  1%positive l.
...
Fixpoint pow_mod_pred (a:N) (l:dec_prime) (n:positive) {struct l} : N :=
  ...
  | (q,p)::l =>
    if (p ?= 1) then pow_mod_pred a l n
      ...
      let a' := iter_pos _ (fun x => Npow_mod x q n) a (Pos.pred p) in
      pow_mod_pred a' l n
  end.
...
Definition check_s_r s r sqrt :=
 ...
 | Npos p =>
    ...
    | Zpos x =>
       ...
       let sqrt12 := square (Pos.succ sqrt)  in
...
Proof.
 unfold Z.lt;destruct p;simpl; auto; red;reflexivity.
Qed.
...
Proof.
 intros;unfold Z.ge;simpl;intro;discriminate.
Qed.
...
Qed.
#[global] Hint Rewrite times_mod_spec : zmisc.
...
Lemma in_mkProd_prime_div_in :
  ...
  forall (l:dec_prime),
    ...
    Z.divide p (mkProd l) -> exists n,In (p, n) l.
Proof.
 induction l;simpl mkProd; simpl In; mauto.
 intros _ H1; absurd (p <= 1).
 apply Zlt_not_le; apply Z.lt_le_trans with 2; try apply prime_ge_2; auto with zarith.
...
Proof.
 ...
 destruct ((Pos.pred N) / (mkProd dec))%P as (R1,n); mauto;intros (H2,H3).
