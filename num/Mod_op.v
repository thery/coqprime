...
Section Mod_op.
 ...
 Let ww_lsl_n :=
   ...
   | N0 => fun ww => ww
   | Npos p => 
     fun ww => ww_add_mul_div w0 wWW wW0 w0W w_add_mul_div w_digits p ww W0
   end.
...
*)
 ...
 rewrite (Zmod_def_small (- [|b|] + base (znz_digits w_op)));auto with zarith.
 ...
 assert (HHH := spec_to_Z op_spec b);repeat rewrite Zmod_def_small;auto with
 zarith.
 ...
 change
   ([[ww_add_mul_div w0 wWW (znz_W0 w_op) (znz_0W w_op) 
        w_add_mul_div w_digits
        p w2 W0]] =  2 ^ Zpos p * [[w2]]).
 ...
 change [[W0]] with 0. rewrite Zdiv_0; auto with zarith.
...
(*********** Mersenne special **********)
 ...
 Let add_mul_div :=  ww_add_mul_div w0 wWW wW0 w0W w_add_mul_div w_digits.
 ...
 Let _mmul_mod x y :=
  ...
  | WW xh xl =>
      let xl1 := w_pos_mod p xl in
        match add_mul_div p1 W0 xy with
 ...
 Theorem mmul_aux4:forall x,
  [[x]] < [|b|] * 2 ^  Zpos p ->
     match ww_add_mul_div w0 wWW wW0 w0W w_add_mul_div w_digits p1 W0 x with
          W0 => 0
        | WW _ xl2 => [|xl2|]
     end = [[x]] / 2 ^ Zpos p.
 intros x Hx.
 ...
 red; simpl; auto.
 match goal with |- context[ww_add_mul_div w0 wWW wW0 w0W 
   w_add_mul_div w_digits ?y ?z ?t] =>
   generalize (@spec_ww_add_mul_div w w0 wWW wW0 w0W 
   w_add_mul_div w_digits w_to_Z (spec_0 op_spec) (spec_to_Z op_spec)
   (spec_WW op_spec) (spec_W0 op_spec) (spec_0W op_spec) (spec_add_mul_div op_spec) z t y U);
   case (ww_add_mul_div w0 wWW wW0 w0W 
   w_add_mul_div w_digits y z t)
 ...
 case (spec_ww_to_Z w_digits w_to_Z (spec_to_Z op_spec) x); auto with zarith.
 unfold base.
 apply Zdiv_lt_upper_bound; auto with zarith.
 case (spec_ww_to_Z w_digits w_to_Z (spec_to_Z op_spec) x); auto with zarith.
 rewrite <- Zpower_exp; auto with zarith.
 ...
 Theorem mmul_aux5:forall xh xl, 
      ...
      let xl1 := w_pos_mod p xl in
      let r := 
        match ww_add_mul_div w0 wWW wW0 w0W w_add_mul_div w_digits p1 W0 (WW xh xl) with
          W0 => match w_compare xl1 b with
                | Lt => xl1
                | Eq => w0
                | Gt => w1
 ...
 case (spec_ww_to_Z   w_digits w_to_Z (spec_to_Z op_spec) (WW xh xl)); simpl; auto with zarith.
 Qed.
