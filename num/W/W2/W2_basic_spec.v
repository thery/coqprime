...
Lemma w2_W0_spec : forall h, [||w2_W0 h||] = [|h|] * w2_B.
Proof
fun h =>
 match h as h return [||w2_W0 h||] = [|h|] * w2_B with
 | OO => refl_equal 0
 | OI => refl_equal ([||w2_W0 OI||])
 | IO => refl_equal ([||w2_W0 IO||])
 | II => refl_equal ([||w2_W0 II||])
 end.

Lemma w2_0W_spec : forall l, [||w2_0W l||] = [|l|].
Proof
fun l =>
 match l as l return [||w2_0W l||] = [|l|] with
 | OO => refl_equal ([||w2_0W OO||])
 | OI => refl_equal ([||w2_0W OI||])
 | IO => refl_equal ([||w2_0W IO||])
 | II => refl_equal ([||w2_0W II||])
 end.
